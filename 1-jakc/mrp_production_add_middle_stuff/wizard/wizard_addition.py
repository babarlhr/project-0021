...
class WizProductionProductLine(models.TransientModel):
    ...
    type = fields.Selection([('add','Addition'),('return','Return')],'Type', required=True, default='add')
    ...
    def _prepare_product_addition(self, product, product_qty, production):
        addition_vals = {'product_id': product.id,
                         ...
                         'addition': True if self.type == 'add' else False,
                         'deduction': True if self.type == 'return' else False}
    ...
    def add_product(self):
        ...
        if self.product_qty <= 0:
            raise Warning(_('Warning'), _('Please provide a positive quantity'))
