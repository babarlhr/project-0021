...
class HrEmployeePeriodeFee(models.Model):
    ...
    @api.one
    def paid_operator_fee(self):
        for fee in self:
            for workcenter_line in fee.workcenter_line_ids:
                workcenter_line.paid_state = 'paid'
            fee.state = 'done'
    ...
    def process_operator_fee(self):
        ...
        for wc_line_operator in wc_line_operator_ids:
            ...
            if wc_line_operator.iface_operator_fee:
                vals.update({'fee_amount': wc_line_operator.operator_fee})
            else:
                vals.update({'fee_amount': 0})
            vals.update({'state': 'done'})
            wc_line_operator.write(vals)

    @api.one
    def re_process_operator_fee(self):
        for workcenter_line in self.workcenter_line_ids:
            workcenter_line.fee_amount = workcenter_line.operator_fee
