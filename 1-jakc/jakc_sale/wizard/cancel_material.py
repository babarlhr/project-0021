...
class WizardCancelMaterial(models.TransientModel):
    _name = 'wizard.cancel.material'
    ...
    iface_cancel_only = fields.Boolean('Cancel Only', default=False)
    product_qty = fields.Float('Product Quantity', required=False, default=1)
    product_uom = fields.Many2one('product.uom', 'Product Unit of Measure', required=False)

    @api.model
    def default_get(self, fields):
        res = super(WizardCancelMaterial, self).default_get(fields)
        sale_order_consume_material_obj = self.env['sale.order.consume.material']
        active_id = self.env.context.get('active_id') or False
    ...
    def cancel_material(self):
        ...
        if consume_material.quantity_usage != 0:
            raise exceptions.Warning(_('Please Return Material Before Adjustment Material'))

        #Cancel Product to Consume
        for line_id in consume_material.line_ids:
            stock_move_id = line_id.stock_move_id
            if stock_move_id.state != 'done':
                stock_move_id.action_cancel()

        if not self.iface_cancel_only:
        ...
        if not self.iface_cancel_only:
            consume_material.sale_order_id.message_post(body=_("""Adjust Material {} for {} {} """.format(self.product_id.name, self.product_qty, self.product_uom.name)))